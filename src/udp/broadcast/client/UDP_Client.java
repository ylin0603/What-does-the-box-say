...
public class UDP_Client {
	...
	private ArrayList<EncodedData> encapsulateData() {
		ArrayList<ClientPlayerFeature> updatePlayers = Cdc.getInstance().getPlayersUpdateInfo();
		ArrayList<ClientItemFeature> updateItems = Cdc.getInstance().getItemsUpdateInfo();
	...
	private void broadcast(ArrayList<String> allIP, ArrayList<EncodedData> encodedData) {
		...
		try {
			for(String ip : allIP){
				...
				DatagramPacket sendPacket
                        = new DatagramPacket(sendData, sendData.length, IPAddress, 3335);
