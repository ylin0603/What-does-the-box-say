...
public class GuiStatusBar extends GuiComponent {
    ...
    public GuiStatusBar() {
        ...
        playerName =
                new GuiLabel("Player Name : ", x + 230, y + 20, Color.WHITE);
        weaponName = new GuiLabel("Weapon : " + weapon, x + 430, y + 20,
                Color.WHITE);
    ...
    public void update() {
        ...
        roomOwner.setText(
                "Room Owner : " + dom.getCharacterList().get(0).getNickName()); // DOM.get
        playerName.setText("Player Name : " + player.getNickName());
        setWeopon(player.getWeaponType());
        bulletCount.setText("Bullet : " + player.getBulletCount());
    }

    public void setWeopon(int weaponType) { // should change character's weapon type
        if (weaponType == 0)
            weapon = KNIFE;
        else
            weapon = GUN;
